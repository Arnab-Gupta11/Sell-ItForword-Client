  ...
  const onSubmit = async (data: ICategoryFormData) => {
    try {
      ...
      const categoryInfo = {
        ...
        icon: imageData || category?.icon,
        description: data.description || category?.description,
      };
      const res = await updateCategory(category?._id, categoryInfo);
      if (res?.success) {
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <span className="hover:text-primary border-2 border-[#e9ebec] dark:border-[#142e3a] py-2 px-5 rounded-lg hover:bg-light-primary-bg dark:hover:bg-dark-secondary-bg font-medium text-sm w-full cursor-pointer">
          <span>Update</span>
        </span>
      </DialogTrigger>
        ...
        <form onSubmit={handleSubmit(onSubmit)} autoComplete="off">
          ...
          <div className=" mt-8">
            <Button variant={"primary"} type="submit" disabled={loading} className="sm-mx:w-full w-32">
              {loading ? <BiLoaderCircle className="animate-spin" /> : "Update Category"}
            </Button>
          </div>
        </form>
      </DialogContent>
